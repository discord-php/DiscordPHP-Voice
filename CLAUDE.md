...
- **`VoiceClient` behaviour lives in method-only traits.** `Discord\Voice\Concerns\{ManagesPlayback, ManagesReceiving, ManagesRecording}` hold the method bodies; every property, constant and the constructor stay on `VoiceClient`. Add new methods to the matching concern; keep state on the class so reflection-based tests and `WS`/`UDP` pokes keep working.
